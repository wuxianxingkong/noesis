...
public class TestDrawing {
	...
	public static void main(String[] args) 
	{
		...
		if (background) {
			...
			drawing.add ( new Rectangle("backgrount", back, null, 0, 0, 600, 600));
		}
		
		// External image 
		// drawing.add( new Bitmap("http://elvex.ugr.es/image/logo/decsai.png", 100, 100, 187, 219) );

		// Geometrical shapes
		
		color = new Style(new Color(0x00, 0x70, 0x70, 0x80),3);
		drawing.add ( new Rectangle("rect", color, null, 10, 10, 150, 30));
		color = new Style(new Color(0x70, 0x00, 0x00, 0x80),3);
		drawing.add ( new Polygon("rect", color, color, new int[]{15, 15, 80, 145, 145}, new int[]{15,50,80,50,15}));
		color = new Style(new Color(0x70, 0x70, 0x70, 0x80),3);
		drawing.add ( new Ellipse("rect", color, null, 80, 80, 50, 30));
		color = new Style(new Color(0x00, 0x70, 0x70, 0x80),3);
		drawing.add ( new Text("Figura en formato vectorial", color, 8, 10));
		
		for (int i=0; i<=90; i+=10) {
			FontStyle font = new FontStyle ( new Color(0x70, 0x70, 0x70, 0x80), new Font("Serif", Font.PLAIN, 24), -i);
			drawing.add ( new Text("Figura en formato vectorial", font, 20, 550));
		}
		...
		if (flat) {
			...
			Style border = new Style ( new Color(0x33, 0x00, 0x00, 0xFF), 0);
	...
	public static void display (Drawing drawing)
	{
		JFrame jframe = new JFrame("Drawing...");
		
		jframe.setSize(600,600);
		jframe.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		jframe.add ( new JDrawingComponent(drawing));
